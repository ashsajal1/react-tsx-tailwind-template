...
const InputField = forwardRef<HTMLInputElement, InputProps>((props, ref) => {
    ...
    return (
        <input {...rest} ref={ref} className={cn("p-2 border  outline-none rounded dark:text-white bg-white dark:bg-black dark:border dark:border-gray-800 focus:border-primary", className, { [btnClass]: (type === 'submit') })} title="Input field" />
    );
});
