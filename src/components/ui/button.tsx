...
export default function Button({ children, variant = "solid", ...props }: PropsWithChildren<ButtonProps>) {
    ...
    const buttonStyles = "flex items-center justify-center gap-2 cursor-pointer select-none p-2 rounded transition duration-0 text-white transform active:scale-[102%] min-w-[60px]";
    ...
    return (
        <button
            {...props}
            className={cn(buttonStyles, className, { [dangerStyle]: variant === 'danger', [dangerOutlineStyle]: variant === 'danger-outline', ['bg-gradient-to-br from-primary to-primary-high hover:to-primary-extra-high']: variant === 'solid', [ghostStyle]: variant === "ghost", [outlineStyle]: variant === "outline" })}
        >
            {children}
        </button>
    );
